...
int szyfrowanie_rekurencyjne(char tekst_szyfrowany, const int metoda_szyfrowania)
{
...
void deszyfrowanie( string &encrypted, const int decrypt_method )
{
	...
	for(int i=0;i<encrypted.size();i++)
	{
		...
		if(szyfrowanie_rekurencyjne(32, decrypt_method)	==	encrypted[i]) // moze to spacja?
			encrypted[i]=szyfrowanie_rekurencyjne(32, decrypt_method);
		else
		for(int s=48; s<=57; s++) // moze to jakas liczba?
		{
			char check=szyfrowanie_rekurencyjne(i, decrypt_method);
			cout<<"jestem w liczbach"<<endl;
		...
		{
			char check=szyfrowanie_rekurencyjne(i, decrypt_method);
			cout<<"jestem w duzych literach";
			if(check == encrypted[i])
			{
			cout<<"ZNALEZIONA LITERA TO: "<<check<<endl;
			encrypted[i]=check;
			break;
			}
		}
